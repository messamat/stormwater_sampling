...
bingcleanout = 'bingmeanclean1806'
...
#Clean out road closure artefacts
bingclean = Con(Raster(bingmeanout)<2.5, Raster(bingmeanout), 0)
bingclean.save(bingcleanout)

#Generate heat map
heat_bing = FocalStatistics(path.join(gdb,bingcleanout), neighborhood=kernel, statistics_type='SUM', ignore_nodata='DATA') #NEed to be in geodatabse
