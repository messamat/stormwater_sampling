...
Required Arguments:
    - dir (path): directory containing weighted kernel text files
...
def customheatmap(dir, in_raster, out_var, divnum=1, keyw=''):
    for kertxt in os.listdir(dir):
        if re.compile('kernel' + keyw).match(kertxt):
            ...
            if not arcpy.Exists(outext):
                print(outext)
                kernel = arcpy.sa.NbrWeight(os.path.join(dir, kertxt))
